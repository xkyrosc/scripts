--FH
if getgenv().ZyrtecHub_Running then
...
getgenv().ZyrtecHub_Running = true
...
local function PostToWebhook()
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = string.format("ZyrtecHub â€¢ Session: %s â€¢ %s", GetSessionTime(), os.date("%x %X")) },
...
local function SendSafetyWebhook(targetPlayer, reason)
    ...
    local payload = {
        ["embeds"] = {{
            ...
            ["footer"] = { ["text"] = "ZyrtecHub â€¢ " .. os.date("%x %X") }
        }}
    }
            ...
            if url ~= "" and url:find("discord.com/api/webhooks/") then
                local payload = {
                    ["embeds"] = {{
                        ...
                        ["footer"] = { ["text"] = "ZyrtecHub | Sailor Piece" }
                    }}
                }
                task.spawn(function()
                    pcall(function()
...
local Window = Library:CreateWindow({
	Title = "ZyrtecHub | SP",
	Footer = "" .. assetName .. " | by zyrtechub | Sailor Piece | Version 1.5 Beta",
	NotifySide = "Right",
...
task.spawn(function()
    while task.wait(5) do
        if not getgenv().ZyrtecHub_Running then break end
...
task.spawn(function()
    while getgenv().ZyrtecHub_Running do
...
MenuGroup:AddButton("Unload", function()
    getgenv().ZyrtecHub_Running = false
    Shared.Farm = false
    Cleanup(Connections)
    Cleanup(Flags)
	Library:Unload()
...
ThemeManager:SetFolder("ZyrtecHub | Sailor Piece")
SaveManager:SetFolder("ZyrtecHub/SailorPiece")
